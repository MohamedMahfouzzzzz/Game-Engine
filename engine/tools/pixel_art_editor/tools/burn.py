...
class BurnTool(ToolBase):
    """Darken/burn tool for shadows."""

    def __init__(self):
        super().__init__("Burn", icon="burn")
        ...
        if self._is_drawing and self._active_layer:
            for dy in range(-self._brush_size, self._brush_size + 1):
                for dx in range(-self._brush_size, self._brush_size + 1):
                    ...
                    # Darken
                    factor = max(0.0, 1.0 - self._exposure * self._opacity)
                    new_color = (
